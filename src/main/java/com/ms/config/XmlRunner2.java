	...
	*/
	...
	public static void main(String[] arg) throws Exception {
		...
		for (Entry<String, JsonElement> entry : brConfig) {
			i++;
			final CucumberOptions classAnnotation = TestRunner2.class.getAnnotation(CucumberOptions.class);
			...
			threads[i - 1] = new Thread(new XmlRunner2().new MyRunnable());
			threads[i - 1].start();
	...
	public void failedRerun(String browser, Long oldThread) {
		...
		Thread thread_Failed = new Thread(new XmlRunner2().new failedRunnable());
		thread_Failed.start();
